...
export type TContactListParams = {
  cursor?: string;
  limit?: number;
  search?: string;
  tag?: string;
...
export type TDiskSpaceResponse = {
...
export function getContacts(params?: TContactListParams): Promise<TContactListResponse> {
  ...
  const query = Object.keys(cleanParams).length > 0
    ...
    : '';
  return request.get(`${endpoints.contacts()}${query}`);
}

export function searchContacts(query: string, limit?: number, field?: string): Promise<{ contacts: TContact[] }> {
  return request.post(endpoints.searchContacts(), { query, limit, field });
}

export function getContact(id: string): Promise<TContact> {
  return request.get(endpoints.contactById(id));
}

export function createContact(payload: Partial<TContact>): Promise<TContact> {
  return request.post(endpoints.contacts(), payload);
}

export function updateContact(id: string, payload: Partial<TContact>): Promise<TContact> {
  return request.put(endpoints.contactById(id), payload);
}
...
export function checkContactDiskSpace(): Promise<TDiskSpaceResponse> {
...
export function getImportStatus(jobId: string): Promise<TImportJob> {
  return request.get(endpoints.importContactsStatus(jobId));
}
